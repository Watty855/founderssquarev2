...
interface UndoBuildDialogProps {
  open: boolean
  propertyName: string
  plotLocation: string
  buildCost: number
...
export function UndoBuildDialog({
  open,
  propertyName,
  plotLocation,
  buildCost,
    ...
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onCancel()}>
      ...
      >
        <DialogHeader style={{ marginBottom: 4 }}>
          <DialogTitle style={{ fontSize: 18, fontWeight: 400 }}>Undo Build?</DialogTitle>
          <DialogDescription style={{ fontSize: 13, color: '#8888a0', lineHeight: 1.4 }}>
            Remove <strong style={{ color: '#f0f0f5' }}>{propertyName}</strong> from{' '}
            <strong style={{ color: '#f0f0f5' }}>{plotLocation}</strong>? You will be refunded ${buildCost}M.
          </DialogDescription>
        </DialogHeader>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button
            ...
            className="btn-ps"
            style={{
              flex: 1, height: 42, borderRadius: 10, backgroundColor: '#c81b3a', color: '#fff',
